...
void testBigint(SAConnection& con)
{
	SACommand cmd;
	SAValueRead v(SA_dtBLob);
	SAString s;
	SANumeric num;
	SAInterval interval;
	size_t n = sizeof(s);
	...
	//cmd.setCommandText(_TSA("select f1,f2 from t1 where f1=:0"));
	//cmd << SAPos(0) << (sa_int64_t)1234567890123456;
	//cmd.Param(_TSA("unicode")).setAsInt64() = (sa_int64_t)1234567890123456;

	/*
	SADateTime val = SADateTime::currentDateTimeWithFraction();
	cmd.Param(1).setAsDateTime() = val;
	*/
	...
	while (cmd.isResultSet()) {
		while (cmd.FetchNext())
		{
			printf("f1=%s\tf2=%s\n", cmd[1].asString().GetMultiByteChars(), cmd[2].asString().GetMultiByteChars());
			cmd.Cancel();
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	SAConnection con;
	SAConnection con1;
	cmd.setConnection(&gcon);

	try
	{
		api.setClient(SA_CubeSQL_Client);

		gcon.setAPI(&api);

		//api.setOption(_TSA("UseAPI")) = _TSA("OLEDB"); // force SQLAPI to use OLEDB instead of ODBC
		//api.setOption(_TSA("CoInitializeEx_COINIT")) = _TSA("Skip");
		//api.setOption(_TSA("OLEDBProvider")) = _TSA("CompactEdition.4.0");
		...
		*/
		...
		//con1.Connect(_TSA("ora12c"), _TSA("user1"), _TSA("java"), SA_Oracle_Client);
		...
		//api.setClient(SA_MySQL_Client);
		//printf("Client set!\n");
		//con.Connect(_TSA("localhost@test"), _TSA("root"), _TSA(""));
		//con.Connect(_TSA("server=demo17"), _TSA("DBA"), _TSA("sql"), SA_SQLAnywhere_Client);
	

		const char* TEST_STRING = "\xf0\x9f\x91\xa9\xf0\x9f\x8f\xbc\xe2\x80\x8d\xf0\x9f\x9a\x92 12345678";
		
		gcon.Connect(_TSA("localhost@test"), _TSA("admin"), _TSA("admin"));		
		cmd.setCommandText(_TSA("update t2 set f2=:1 where f1=1"));
		cmd.Param(1).setAsString().SetUTF8Chars(TEST_STRING);
		//cmd.Execute();

		//gcon.Connect(_TSA("localhost@mysql"), _TSA("root"), _TSA(""));
		printf("Connected!\n");		

		//gcon.setAutoCommit(SA_AutoCommitOff);

		//cmd.setCommandText(_TSA("select f2 from t2 where f1=1"));
		cmd.setCommandText(_TSA("SHOW DATABASES"));
		cmd.Execute();
		while (cmd.FetchNext())
		{
			SAString s = cmd[1].asString();
			const char* sz = s.GetUTF8Chars();
			printf("%s\n", sz);
			printf("Result is %s\n", 0 == strcmp(TEST_STRING, sz) ? "OK" : "FALSE");
		}

		//con.setIsolationLevel(SA_Snapshot);
...
int main2(int argv, char *argc[])
{
	...
	try
	{
		...
		while (cmd.FetchNext())
		{
			cmd.Field(1).setFieldType(SA_dtLong);
		}
		time(&finish);
